...
async def decrypt_and_merge_video(mpd_url, keys_string, output_path, output_name, quality="720"):
    try:
        ...
        # Use system mp4decrypt if available, otherwise use local installation
        mp4decrypt_path = "mp4decrypt"
        if not os.path.exists("/usr/local/bin/mp4decrypt"):
            mp4decrypt_path = os.path.expanduser("~/.local/bin/mp4decrypt")
        
        for data in avDir:
            if data.suffix == ".mp4" and not video_decrypted:
                cmd2 = f'{mp4decrypt_path} {keys_string} --show-progress "{data}" "{output_path}/video.mp4"'
                ...
                cmd3 = f'{mp4decrypt_path} {keys_string} --show-progress "{data}" "{output_path}/audio.m4a"'
